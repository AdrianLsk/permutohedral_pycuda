...
// template<int kd>
__global__ static void cleanHashTable(const int kd, int n, MatrixEntry
*matrix) {
...
// template<int {{ pd }}, int {{ vd }}>
__global__ static void blur(int n, float *newValues, int color, MatrixEntry *matrix) {
    const int idx = (blockIdx.y * gridDim.x + blockIdx.x) * blockDim.x * blockDim.y + threadIdx.x;
...
void filter_(float *im, float *ref, int w, int h, bool accurate) {
    ...
    MirroredArray<float> values(im, n*{{ vd }}); // data, size
...
//    createMatrix<<<blocks, blockSize>>>({{ pd }}, w, h, positions.device,
    ...
    cleanHashTable<<<cleanBlocks, cleanBlockSize>>>({{ pd }}, 2*n*({{ pd }}+1), matrix
    .device);
...
//    splatCache<{{ pd }}, {{ vd }}><<<blocks, blockSize>>>(w, h, values.device, matrix.device);
	...
	for (int color = 0; color <= {{ pd }}; color++) {
	    blur<<<cleanBlocks, cleanBlockSize>>>(n*({{ pd }}+1), newValues, color,
	    matrix.device);
