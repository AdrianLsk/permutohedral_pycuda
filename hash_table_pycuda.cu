...
void createHashTable(int kd, int vd, int capacity) {
...
// gives warning that the variables table_values, table_capacity cannot be read
// directly in the host function - not a problem, since it's not used
//template <int vd>
static void resetHashTable(int vd) {
...
//template<int kd>
__device__ __host__ static unsigned int hash(int kd, signed short
*key) {
    unsigned int k = 0;
    for (int i = 0; i < kd; i++) {
	k += key[i];
...
//template<int kd>
__device__ __host__ static unsigned int hash(int kd, int *key) {
    unsigned int k = 0;
    for (int i = 0; i < kd; i++) {
	k += key[i];
	k = k * 2531011;
...
//template<int d>
__device__ static bool matchKey(int d, int idx, signed short * key) {
    bool match = true;
...
//template<int d>
__device__ static void generateKey(int d, int idx, signed short * key) {
    int slot = idx/(d+1), color = idx-slot*(d+1);
    char *rank = table_rank + slot * (d+1);
    signed short *zero = table_zeros + slot * (d+1);
    for (int i = 0; i < d; i++) {
...
//template<int kd>
__device__ static int hashTableInsert(int kd, unsigned int fh, signed short *key, unsigned int slot) {
    int h = modHash(fh);
    while (1) {
...
//template<int kd>
__device__ static int hashTableInsert(int kd, signed short *key, unsigned int slot) {
...
//template<int kd>
__device__ static int hashTableRetrieveWithHash(int kd, unsigned int fh, signed short *key) {
  int h = modHash(fh);
  while (1) {
    int *e = table_entries + h;
...
//template<int kd>
__device__ static int hashTableRetrieve(int kd, signed short *key) {
